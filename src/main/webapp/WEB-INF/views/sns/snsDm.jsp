...
<body>
	...
	function makeDM(json) {
		...
		make+='<img src="'+json[k]["memberPic"]+'" class="img-thumbnail';
		make+=' img-responsive">';
		make+='</td>';
		make+='<td class="col-xs-10 col-sm-10 col-md-11 col-lg-11">';
		make+='<div class="headBox">';
		make+='<div class="userId">'+json[k]["getMember"]+'</div>';
		make+='<div class="contentDate">';
		make+=json[k]["dmDate"]["date"]["year"]
		make+='-'+json[k]["dmDate"]["date"]["month"]+'-'+json[k]["dmDate"]["date"]["day"]
		make+=' '+json[k]["dmDate"]["time"]["hour"]+':'+json[k]["dmDate"]["time"]["minute"]
		make+='</div>';
		make+='</div>';
		make+='<br/>';
		make+='<div class="contentBox">';
		make+='<div class="content">'+json[k]["contents"]+'</div>';
		make+='<div class="contentCnt">1</div>';
		make+='</div>';
		make+='</td>';
		make+='</tr>';
		}
